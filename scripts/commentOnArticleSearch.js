...
(async () => {
  await runWithCore(async ({ core, wantsBackground }) => {
    ...
    const commentPrefix = `${selectedOpeningPhrase} コメントさせていただきます✨フォロー・スキもさせていただきました💫`;
    const commentSuffix =
      `${selectedSuffixOpeningPhrase}、私の記事にもコメント・フォロー・スキをいただけますと大変励みになります😊`;
