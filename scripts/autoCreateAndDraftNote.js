...
(async () => {
  await runWithCore(async ({ core, wantsBackground }) => {
    ...
    const systemMessage =
      'あなたはチョウピラコ様や座敷わらし、スピリチュアルな世界に詳しい日本語のnote記事編集者です。応答は「生成した記事の内容だけ」を出力してください。出力は「# タイトル」1行から始まり、続けて本文、最後にハッシュタグのみとし、説明・前置き・後付け・構成案・「以下、本文を作成します」「最終的に、以下のようになります」「この内容は〜」などのメタ説明やnoteのページUIは一切含めないでください。';
    ...
    const articleConditionsLines = [
      ...
      '【禁止】出力に以下を含めないでください。「〇〇 投稿」「見出し画像」などのページUI表記、クリエイター名・筆者名・アカウント名、日付・時刻、noteのフッターリンク（noteプレミアム・note pro・よくある質問・プライバシー・ご利用規約等）、ブラウザのタブタイトル風の「タイトル｜クリエイター名」形式。依頼内容の復唱や承諾の言葉（「わかりました」「次に、本文を作成します」等）。「本文の構成案：」「以下、本文を作成します：」「この内容は〜な内容になっています」「最終的に、以下のようになります：」などの説明・前置き・後付け。コメント募集文（「✨コメントを頂けますと〜」等）。タグ生成の説明（「この記事の内容から、関連する日本語タグを〜」等）。',
      '【出力形式】応答の先頭1文字目から「# 記事タイトル」の行を書き、改行のあと本文、最後に改行とハッシュタグ（#〇〇 #〇〇 ...）のみ。上記以外の文字は1文字も出力しないでください。',
    ];
